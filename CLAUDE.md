...
```
alchemist-weaklabel (Python, zigzag segmenter)      alchemist-labeler (this repo)
    │                                                       │
    │  build_blind_precompute (no weak-label opinions)      │
...
No build coupling. No shared library.

> The precompute producer is **alchemist-weaklabel** (`scripts/export_blind_pilot_precompute.py`,
> ATR-ZigZag segmenter). The legacy C++ `alchemist/tools/pl-export` path was
> **removed** — its SMA-smoothed PL segmentation merged deep V/Λ moves and it
> mis-named its provenance `pl_proposal_version`. Provenance is now the
...
|---|---|---|---|
| A | Pre-compute producer | `alchemist-weaklabel` (Python) | `scripts/export_blind_pilot_precompute.py` → blind precompute (bars + segment grid + ht_trendline, no opinions). Superseded the removed opt-in C++ `tools/pl-export`. |
...
```json
{
  ...
  "segmenter": {
    ...
    "params_hash": "<hash of the segmenter params>",
    "code_git_sha": "<sha of alchemist-weaklabel at run time>"
  },
  "windows": [
    {
...
- `public/precomputes/` — gitignored (regenerated by `alchemist-weaklabel`)
...
Segmentation parameters live in `alchemist-weaklabel` (the ATR-ZigZag
`WeakLabelConfig`); the precompute records them indirectly via the `segmenter`
block's `params_hash`, not as named fields in the JSON contract.
...
6. **Reviewer provenance + version pinning.** Export schema carries
   ...
   `segmenter` (`algo`/`version`/`params_hash`/`code_git_sha`), `label_set_version`, and
